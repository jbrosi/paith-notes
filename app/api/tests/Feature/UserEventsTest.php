...
it('records login event on auth callback', function (): void {
    ...
    $stmt->execute([':user_id' => $userId, ':event' => 'login', ':meta' => json_str(['ip' => '1.2.3.4'])]);
...
it('records logout event and returns it in events feed', function (): void {
    ...
    $encrypted = $crypto->encrypt((string)json_str(['access_token' => 'tok']));
...
it('supports pagination for events', function (): void {
    ...
    for ($i = 0; $i < 5; $i++) {
        $stmt->execute([':uid' => $userId, ':event' => 'login', ':meta' => json_str(['n' => $i])]);
    }
