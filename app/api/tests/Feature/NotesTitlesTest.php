...
function titlesSetup(string $idPart): array
{
    ...
    $res = App::handle('POST', '/api/nooks', $headers, json_str(['name' => 'T']));
...
it('returns a lean note projection (id/nook_id/title/type_id/version)', function (): void {
    ...
    App::handle('POST', "/api/nooks/{$nookId}/notes", $headers, json_str(['title' => 'First']));
    App::handle('POST', "/api/nooks/{$nookId}/notes", $headers, json_str(['title' => 'Second']));
...
it('honours ?limit and caps at 200', function (): void {
    ...
    for ($i = 0; $i < 25; $i++) {
        App::handle('POST', "/api/nooks/{$nookId}/notes", $headers, json_str(['title' => "Note {$i}"]));
    }

    // Explicit limit honoured
    $res = App::handle('GET', "/api/nooks/{$nookId}/notes?lean=1&limit=10", $headers, '');
    expect(count(json_body($res)['notes']))->toBe(10);

    // Well over max stays within 200
    $res = App::handle('GET', "/api/nooks/{$nookId}/notes?lean=1&limit=9999", $headers, '');
    expect(count(json_body($res)['notes']))->toBeLessThanOrEqual(200);
});

it('filters by case-insensitive title substring when q is supplied', function (): void {
    [$headers, $nookId] = titlesSetup('333333333333');
    App::handle('POST', "/api/nooks/{$nookId}/notes", $headers, json_str(['title' => 'Apple Pie']));
    App::handle('POST', "/api/nooks/{$nookId}/notes", $headers, json_str(['title' => 'Banana Bread']));
    App::handle('POST', "/api/nooks/{$nookId}/notes", $headers, json_str(['title' => 'apple cider']));

    $res = App::handle('GET', "/api/nooks/{$nookId}/notes?lean=1&q=" . urlencode('apple'), $headers, '');
    expect($res['status'])->toBe(200);
    $titles = array_column(json_body($res)['notes'], 'title');
