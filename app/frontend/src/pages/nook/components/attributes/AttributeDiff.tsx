...
const DIFF_RENDERERS: Partial<Record<string, DiffRenderer>> = {
	...
	number: (attr, oldValue, newValue) => {
		...
		const oldNum = typeof oldValue === "number" ? oldValue : Number(oldValue) || 0;
		...
		const stars = (n: number, color: string) => (
			<span>
				{Array.from({ length: maxVal }, (_, i) => (
					<span style={{ color: i < n ? color : "var(--color-border-light)", "font-size": "0.85rem" }}>★</span>
				))}
			</span>
		);
		...
		return (
			<div style={{
				...
				<span style={{ opacity: "0.6" }}>{stars(oldNum, "var(--seed-warning)")}</span>
				<span style={{ color: "var(--color-text-muted)", "font-size": "0.75rem" }}>→</span>
