...
export function AttributeDiffValue(props: {
	...
	const emptyStyle = { "font-size": "0.8rem", color: "var(--color-text-muted)", "font-style": "italic" as const };
	...
	switch (props.attr.kind) {
		...
		case "number": {
			if (display() === "rating") {
				...
				const numVal = typeof val() === "number" ? (val() as number) : Number(val()) || 0;
				return (
					<span style={{ "font-size": "0.85rem" }}>
						{Array.from({ length: maxVal }, (_, i) => (
							<span style={{ color: i < numVal ? "var(--seed-warning)" : "var(--color-border-light)" }}>★</span>
						))}
						<span style={{ "font-size": "0.75rem", color: "var(--color-text-muted)", "margin-left": "4px" }}>
							{numVal}
						</span>
					</span>
				);
		...
		case "boolean":
			return <span style={{ "font-size": "0.8rem" }}>{val() ? "Yes" : "No"}</span>;
		...
		case "url":
			return (
				<a
					...
					style={{ "font-size": "0.8rem", color: "var(--link-color, #0066cc)", "word-break": "break-all" as const }}
				>
					{strVal()}
				</a>
			);
		...
		case "graph": {
			...
			const parts: string[] = [];
			if (gv.rootNoteId) parts.push(`root: ${String(gv.rootNoteId).slice(0, 8)}…`);
			if (gv.depth) parts.push(`depth: ${gv.depth}`);
			if (gv.layout) parts.push(`layout: ${gv.layout}`);
			return <span style={{ "font-size": "0.8rem", color: "var(--color-text-secondary)" }}>{parts.join(", ") || JSON.stringify(gv)}</span>;
		}

		default:
			// Fallback: JSON representation
			return (
				<span style={{ "font-size": "0.75rem", "font-family": "monospace", color: "var(--color-text-secondary)" }}>
