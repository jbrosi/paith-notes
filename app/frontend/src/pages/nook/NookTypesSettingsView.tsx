...
export function NookTypesSettingsView(props: NookTypesSettingsViewProps) {
	...
	const [typesExpanded, setTypesExpanded] = createSignal<
	...
	const onAddType = async () => {
		...
		if (!key) return;
		const created = await props.store.createNoteType({
			key,
			label,
			parentId: "",
		});
	...
	const onAddSubtype = async () => {
		...
		if (parentId === "") return;
		const parent =
			props.store.noteTypes().find((t) => t.id === parentId) ?? null;
		const label = window.prompt(
			`Subtype label${parent ? ` (under ${parent.label})` : ""}`,
		);
		if (!label) return;
		const keyDefault = label
			.trim()
	...
	const renderTypeTreeNode = (
		t: { id: string; label: string; parentId: string },
		depth: number,
	) => {
			...
			<div style={{ display: "flex", gap: "12px" }}>
				...
				<div style={{ flex: "1", "min-width": "0" }}>
					<Show
						...
						fallback={
							<div style={{ padding: "12px" }}>Select a type to edit.</div>
						}
					>
