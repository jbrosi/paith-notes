...
export default function MyActivity() {
	...
	const loadActivity = async () => {
		...
		try {
			...
			const res = await apiFetch(`/api/me/activity?${params}`, { method: "GET" });
			if (!res.ok) return;
	...
	const tabStyle = (t: Tab) => ({
		padding: "6px 16px",
		border: "none",
		"border-bottom": tab() === t ? "2px solid var(--link-color, #0066cc)" : "2px solid transparent",
		background: "none",
		cursor: "pointer",
	...
	return (
		...
		<div style={{ padding: "1.5rem", "max-width": "600px", margin: "0 auto" }}>
			<h2 style={{ "margin-bottom": "1rem", "font-size": "1.25rem" }}>My Activity</h2>

			<div style={{ "border-bottom": "1px solid var(--border-color, #eee)", "margin-bottom": "1rem" }}>
				<button type="button" style={tabStyle("activity")} onClick={() => setTab("activity")}>
					Changes
				</button>
				<button type="button" style={tabStyle("sessions")} onClick={() => setTab("sessions")}>
					Sessions
				</button>
			</div>
					...
					<For each={activity()}>
						{(entry) => (
							<div style={{
								...
								<span style={{ color: "var(--text-muted, #888)", "margin-left": "8px", "font-size": "0.75rem" }}>
			...
			<Show when={tab() === "sessions"}>
						...
						{(evt) => (
							<div style={{
								...
								<Show when={evt.meta?.ip}>
									<span style={{ color: "var(--text-muted, #888)", "margin-left": "6px" }}>
										from {evt.meta.ip}
									</span>
								</Show>
								<span style={{ color: "var(--text-muted, #888)", "margin-left": "8px", "font-size": "0.75rem" }}>
				...
				<Show when={!evtLoading() && events().length === 0}>
					<div style={{ color: "var(--text-muted, #888)" }}>No session events yet</div>
				</Show>
			</Show>
		</div>
		</>
